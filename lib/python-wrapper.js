...
class PythonWrapper {
  constructor() {
    ...
    // Windows: `py` launcher is the reliable interpreter entrypoint;
    // bare `python` may be the Microsoft Store alias stub.
    this.pythonPath = process.platform === 'win32' ? 'py -3' : 'python3';
    if (this.pythonPath.includes(' ')) {
      // spawn needs argv0 separate from args
      this._pythonArgs = this.pythonPath.split(' ');
      this.pythonPath = this._pythonArgs[0];
      this._pythonExtraArgs = this._pythonArgs.slice(1);
    }
  }

  _spawnPython(args, options) {
    const extra = this._pythonExtraArgs || [];
    return spawn(this.pythonPath, [...extra, ...args], options);
  }
  ...
  async invoke(command, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      const python = this._spawnPython(args, {
        cwd: this.corePath,
        env: {
          ...process.env,
      ...
      python.on('close', (code) => {
        // Core contract: JSON is on STDOUT for success AND failure;
        // exit 0 ok / 1 failure / 2 findings-met-fail-on gate.
        ...
        try {
          ...
          return reject(new Error(`Python output was not JSON (exit ${code}): ${trimmed.slice(0, 300)} | ${stderr.slice(0, 300)}`));
        }
        if (code === 0 || code === 2) {
          return resolve(parsed);
        }
        return reject(new Error(parsed.error || `Python process exited with code ${code}`));
      });
  ...
  async checkPython() {
    return new Promise((resolve) => {
      const python = this._spawnPython(['--version']);
      let output = '';
