...
class OmniSecTester {
  ...
  async verifyCorePackage() {
    ...
    const cliPy = path.join(corePackagePath, 'omnisectester', 'cli.py');

    if (!await fs.pathExists(cliPy)) {
      this.logger.warn('Core package not found. Installing...');
      await this.installCorePackage();
      return;
    }

    // Version pin check: a stale engine (installed by an older CLI) must be
    // upgraded to the pinned release, otherwise new flags silently no-op.
    const expected = ((pkg.config && pkg.config.engine_release) || '').replace(/^v/, '');
    if (expected) {
      try {
        const initPy = await fs.readFile(
          path.join(corePackagePath, 'omnisectester', '__init__.py'), 'utf8');
        const m = initPy.match(/__version__\s*=\s*["']([^"']+)["']/);
        if (m && m[1] !== expected) {
          this.logger.warn(
            `Core engine ${m[1]} != pinned ${expected}. Upgrading...`);
          await fs.remove(corePackagePath);
          await this.installCorePackage();
          return;
        }
      } catch (_) { /* unreadable version -> keep existing engine */ }
    }
